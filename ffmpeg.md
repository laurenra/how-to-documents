...
Create a text file named `file_list.txt` in the same folder as videos.
Add each file path on a separate line using the format: `file 'filename.mp4'`.
Save and close the text file.
Open the terminal in that folder.
Run the command: `ffmpeg -f concat -safe 0 -i file_list.txt -c copy output.mp4`
